...
install_dependencies() {
    ...
    # Check for each dependency (ffmpeg/ffprobe, jq, bc)
...
# Install the script (modified to use embedded source)
install_script() {
    ...
    # Extract embedded script content and write it to the install path
    # Uses 'sed' to print lines between the two markers (excluding the markers themselves)
    # The "$0" variable refers to the installer script itself.
    sed -n '/^START_OPTIMIZER_SCRIPT$/,/^END_OPTIMIZER_SCRIPT$/{/^START_OPTIMIZER_SCRIPT$/d; /^END_OPTIMIZER_SCRIPT$/d; p}' "$0" > "$install_path"
    
    if [ $? -ne 0 ]; then
        echo -e "${RED}Error: Failed to extract and write optimizer script to $install_path${NC}"
        exit 1
    fi
...
print_completion() {
    ...
    echo -e "  Read the project's ${YELLOW}README.md${NC} for detailed usage instructions"
    echo ""
    echo -e "${CYAN}Issues or Questions?${NC}"
    echo -e "  Please report issues on the project's GitHub page."
    echo ""
}
...
# --- CRITICAL FIX ---
# Exit here to prevent the shell from executing the embedded script code as part of the installer script.
exit 0

# --- START_OPTIMIZER_SCRIPT Marker ---
# The content below this line (excluding the END marker) is the embedded optimizer script.
START_OPTIMIZER_SCRIPT
#!/usr/bin/env bash
...
check_dependencies() {
    ...
    # Note: This check is redundant with the installer's check but kept for standalone use
